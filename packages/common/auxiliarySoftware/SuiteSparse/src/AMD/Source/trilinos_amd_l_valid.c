 ...
 *
 * If any of the above conditions hold, TRILINOS_AMD_INVALID is returned.  If the
 * following condition holds, TRILINOS_AMD_OK_BUT_JUMBLED is returned (a warning,
 * not an error):
 *
 *	row indices in Ai [Ap [j] ... Ap [j+1]-1] are not sorted in ascending
 *	    order, and/or duplicate entries exist.
 *
 * Otherwise, TRILINOS_AMD_OK is returned.
 *
 * In v1.2 and earlier, this function returned TRUE if the matrix was valid
 * (now returns TRILINOS_AMD_OK), or FALSE otherwise (now returns TRILINOS_AMD_INVALID or
 * TRILINOS_AMD_OK_BUT_JUMBLED).
 */
...
{
    Int nz, j, p1, p2, ilast, i, p, result = TRILINOS_AMD_OK ;
	...
	TRILINOS_AMD_DEBUG0 (("column 0 pointer bad or nz < 0\n")) ;
	return (TRILINOS_AMD_INVALID) ;
    }
    for (j = 0 ; j < n_col ; j++)
    {
	...
	{
	    i = Ai [p] ;
	    TRILINOS_AMD_DEBUG3 (("row: " ID "\n", i)) ;
	    if (i < 0 || i >= n_row)
	    {
		/* row index out of range */
		TRILINOS_AMD_DEBUG0 (("index out of range, col " ID " row " ID "\n", j, i));
		return (TRILINOS_AMD_INVALID) ;
	    }
	    if (i <= ilast)
	    {
		/* row index unsorted, or duplicate entry present */
		TRILINOS_AMD_DEBUG1 (("index unsorted/dupl col " ID " row " ID "\n", j, i));
		result = TRILINOS_AMD_OK_BUT_JUMBLED ;
	    }
	    ilast = i ;
	}
    }
